...
#include "src/impl/chacha.hpp"
#include "src/impl/pcg.hpp"
#include <atomic>
#include <cstdlib>
#include <mutex>
#include <random>
...
secure_generate()
{
    thread_local static detail::chacha<20> gen{prng_seed(), make_nonce()};
    return gen();
}
...
fast_generate()
{
    thread_local static detail::pcg gen{make_pcg()};
    return gen();
}
